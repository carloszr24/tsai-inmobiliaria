...
export function Footer() {
  ...
  return (
    <footer className="mt-24 border-t border-stone-200 bg-stone-100 text-stone-600">
      <div className="max-w-7xl mx-auto px-6 md:px-10 py-16">
        <div className="grid grid-cols-1 md:grid-cols-4 gap-12 mb-12">
          ...
          <div>
            ...
            <ul className="space-y-2 text-sm">
              ...
              <li>
                <a href={phoneHref} className="transition-colors hover:text-stone-900">{CONTACT.phone.display}</a>
              </li>
              {hasEmail && (
                <li>
        ...
        <div className="border-t border-stone-200 pt-6 flex flex-col md:flex-row justify-between items-center gap-4 text-xs text-stone-500">
          ...
          <Link href="/admin" className="transition-colors hover:text-stone-900">Panel Admin</Link>
        </div>
      </div>
    </footer>
  )
